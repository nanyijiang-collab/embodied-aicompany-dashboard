...
def generate_founders(persons):
    lines = []
    for p in persons:
        name, title, bio = parse_person_info(p)
        if name:
            bio_str = f", bio: '{bio}'" if bio else ""
            lines.append(f"        {{name: '{name}', title: '{title}'{bio_str}}}")
    if lines:
        return "    founders: [\n" + ',\n'.join(lines) + "\n    ]"
    return ""
...
for doc_name, persons in companies_data.items():
    ...
    founders_str = generate_founders(persons)
    if not founders_str:
        continue

    # 查找公司（在更新区域内）
    search_key = f"'{sys_name}':"
    pos = html_content.find(search_key, start_idx)
    if pos < 0:
        print(f"❌ 公司不存在: {sys_name}")
        continue

    # 检查是否已有 founders
    snippet = html_content[pos:pos+1500]
    has_founders = 'founders: [' in snippet[:500]

    if has_founders:
        # 替换现有的 founders
        old_start = pos + snippet.find('founders:')
        # 找到 founders 块的结束
        end_pos = old_start + 8
        bracket_count = 0
        started = False
        for i in range(end_pos, len(snippet)):
            c = snippet[i]
            if c == '[':
                started = True
                bracket_count += 1
        ...
        html_content = html_content[:old_start] + founders_str + html_content[end_pos:]
        updated_count += 1
        ...
        if scene_pos > 0:
            # 找到 scene 字段的结束（单引号后的逗号或换行）
            scene_end = pos + scene_pos
            for i in range(scene_pos, len(snippet)):
                if snippet[i] == "'" and i > scene_pos + 10:
                    scene_end = pos + i + 1
                    break

            html_content = html_content[:scene_end] + ",\n" + founders_str + "\n" + html_content[scene_end:]
            added_count += 1
