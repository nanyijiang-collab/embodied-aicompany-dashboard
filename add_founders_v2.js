...
// 找到getCompanyInfo函数中companies对象的范围
const funcStart = "'                'Figure AI': {";
const funcEndMarker = "            // 英文名匹配";

// 检查并添加founders到每个公司
let updatedCount = 0;

Object.keys(foundersData).forEach(companyName => {
    const data = foundersData[companyName];
    ...
    // 查找公司定义中是否有founders
    const hasFoundersRegex = new RegExp(`'${companyName}':\\s*\\{[^}]*founders:\\s*\\[`);
    ...
    if (match) {
        const startPos = html.indexOf(match[1]);
        
        // 从公司开始位置向后找，找到investors字段前的位置
        const afterCompany = html.substring(startPos);
        const investorsMatch = afterCompany.match(/investors:\s*\[[\s\S]*?\]\s*,\s*milestones:/);
        
        if (investorsMatch) {
            const investorsStart = startPos + afterCompany.indexOf('investors:');
            
            // 在investors前插入founders
            const insertPos = investorsStart;
            
            // 检查是否需要添加逗号
            const beforeInsert = html.substring(startPos, insertPos);
            const needsComma = !beforeInsert.trim().endsWith(',');
            const commaStr = needsComma ? ',' : '';
            
            html = html.slice(0, insertPos) + '                    founders: [\n' + 
                   data.founders.map(f => `                        {name: '${f.name}', title: '${f.title}', bio: '${f.bio}'},`).join('\n') + 
                   '\n                    ],\n' + 
            ...
            // 如果没有investors，在milestones前插入
            const milestonesMatch = afterCompany.match(/milestones:\s*\[[\s\S]*?\]\s*\}/);
            if (milestonesMatch) {
                const milestonesStart = startPos + afterCompany.indexOf('milestones:');
                const insertPos = milestonesStart;
                ...
                console.log(`已添加(在milestones前): ${companyName}`);
                updatedCount++;
            } else {
                console.log(`无法定位插入位置: ${companyName}`);
            }
        }
    } else {
