...
for company_name, persons in founders_data.items():
    ...
    # 在内容中查找公司名
    search_str = f'name:"{company_name}",'
    idx = html_content.find(search_str)
    ...
    snippet = html_content[idx:end_idx]
    if 'founders:' in snippet:
        print(f"[跳过] {company_name}: 已存在founders")
        continue

    # 在 name:"公司名", 后面插入founders
    # 找到 name:"公司名" 结尾的位置
    name_end = html_content.find('"', idx + 5)  # 跳过 name:"
    # 继续找下一个引号
    name_end = html_content.find('"', name_end + 1)
    # 现在找到逗号
    comma_idx = html_content.find(',', name_end)

    if comma_idx > idx and comma_idx < idx + 100:
        # 在逗号后插入founders
        new_content = html_content[:comma_idx+1] + founders_str + ' ' + html_content[comma_idx+1:]
        html_content = new_content
        print(f"[添加] {company_name}: {len(persons)}位创始人")
        updated_count += 1
    else:
        print(f"[错误] {company_name}: 无法定位插入点")
