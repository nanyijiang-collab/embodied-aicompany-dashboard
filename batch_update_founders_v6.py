...
for doc_name, persons in companies_data.items():
    ...
    # 在公司定义范围内搜索（避免误匹配其他映射对象）
    # 从公司名位置往后找 scene: 字段
    snippet = html[pos:pos+2000]

    # 检查是否已有 founders
    has_founders = 'founders: [' in snippet[:800]
    ...
    if has_founders:
        ...
        old_block = snippet[fs_start:fs_end - pos + fs_start] if fs_end > pos else snippet[fs_start:fs_end]
        ...
        if scene_pos > 0:
            # 找到 scene 字段值的结束（找第二个单引号）
            scene_end = scene_pos + 7  # 跳过 "scene:"
            quote_count = 0
            for i in range(scene_pos + 7, len(snippet)):
                if snippet[i] == "'":
                    quote_count += 1
                    if quote_count == 2:
                        scene_end = pos + i + 1
